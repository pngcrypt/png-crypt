...
void sha256_str(const char* input, SHA256_STR* output)
{
    SHA256_BIN buf;
    sha256_bin(input, &buf);

    (*output)[SHA256_STR_SIZE] = 0;
    for (int i = 0; i < SHA256_BIN_SIZE; i++)
        sprintf((*output)+i*2, "%02x", buf[i]);
}

void sha256_bin(const char* input, SHA256_BIN* output)
{
    memset(output,0,SHA256_BIN_SIZE);
